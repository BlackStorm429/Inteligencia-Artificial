...
def generate_truth_table(n, func):
    ...
    if func == "AND":
        results = np.array([all(row) for row in table], dtype=int)
    elif func == "OR":
        results = np.array([any(row) for row in table], dtype=int)
    else:
        raise ValueError("Func must be 'AND' or 'OR'")
...
func = input("Enter the boolean function (AND/OR): ").upper()
...
# Plotagem dos resultados para função
